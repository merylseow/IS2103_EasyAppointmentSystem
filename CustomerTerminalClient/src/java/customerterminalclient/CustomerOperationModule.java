                ...
                if (spInCriteria.isEmpty()) {
                    ...
                    for (ServiceProviderEntity s : spInCriteria) {
                        if (s.getStatus() == APPROVED) {
                            List<Date> availSlotsForTheDay = serviceProviderSessionBeanRemote.retrieveAvailableTimeSlots(date, s.getServiceProviderId());

                            // there are available slots
                            if (!availSlotsForTheDay.isEmpty()) {
                                Date firstAvailTime = serviceProviderSessionBeanRemote.retrieveFirstAvailableTime(date, s.getServiceProviderId());
                                //Date timef = firstAvailTime.getTime();
                                String strTime = timeFormat.format(firstAvailTime);
        ...
        try {
            ...
            if (!spInCriteria.isEmpty()) {
                ...
                while (response != 0) {
                    ...
                    List<Date> availSlots = serviceProviderSessionBeanRemote.retrieveAvailableTimeSlots(date, s.getServiceProviderId());
                    ...
                    String cDateStr = datef.format(calDate);
                    String cTimeStr = timef.format(calDate);

                    if (availSlots.contains(cTime)) {
                        appointmentSessionBeanRemote.bookAppointment(s.getServiceProviderId(), currentCustomerEntity.getCustomerId(), cDateStr, cTimeStr);
    ...
    public void cancelAppointment() {
        try {
            ...
            try {
                ...
                appointmentSessionBeanRemote.cancelAppointment(appointmentId);
            } catch (AppointmentNotFoundException ex) {
